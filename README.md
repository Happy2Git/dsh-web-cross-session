...
| 能力 | 触发方式 | 机制 |
|---|---|---|
| 侧边栏内容搜索 | Web 侧边栏搜索框直接搜消息正文 | 覆盖 `session-query-sqlite` 行为 `openAt: first-search`；核心 `session.search` 返回每条命中的 `seq`，点击结果会**跳转并高亮**到匹配消息。搜索范围是**用户对话与模型输出**——工具调用与结果（命令、代码片段）不参与匹配 |
...
搜索面板是宿主 Web 侧边栏自带的系统搜索框，插件负责解锁它的内容搜索能力：覆盖 `session-query-sqlite` 为持久化 FTS 索引 + `trigram` 分词，并挂载后台预热。装完即用：侧边栏搜索框匹配标题/工作区（本地）和消息正文（FTS 索引，250ms 防抖），结果按会话展示、每条带一行 snippet。

**点击结果跳转定位（滚动到匹配消息 + 行闪光 + 命中词高亮）需要宿主 core 合并搜索增强**——见下"依赖宿主 core 增强"一节。上游原版 core 的系统搜索只打开会话、不定位；两者都是可用状态，差别在于是否定位到具体消息。
...
系统侧边栏搜索的**事件级跳转定位**（点击结果滚动到匹配消息 + 行闪光 + 命中词高亮）由宿主 core 提供，不是插件实现的——侧边栏搜索框是 core 的 `ui-workspace` 组件，插件作为 cordis 插件无法修改它的行为。插件负责的是数据侧：解锁内容搜索（FTS + trigram）、预热索引、以及 `session.search` 返回命中 `seq`（定位坐标由 core 的搜索链路携带）。

- **上游原版 core**：搜索可用（能搜、能打开会话），不定位到具体消息；
- **合并搜索增强的 core**（分支 `feat/sidebar-search-locate`，含 `session.search` 返回 seq + 侧边栏点击 reveal）：搜索并定位——滚动到命中消息、行闪光约 3 秒、命中词高亮约 5 秒。

插件对两种宿主都兼容：检测到宿主支持 `locate` 则完整定位，否则退化为只打开会话。要在你的部署里获得完整定位能力，需将上述 core 增强分支合并进宿主 core。
...
- 首次搜索需全量构建一次索引（耗时 ∝ 历史会话量，见上文；默认由启动后的后台预热承担）；构建是单事务，中断即回滚重来；
...
- 内容搜索召回是 token/phrase 级，无模糊/前缀扩展；结果上限 20 条，点击跳转到命中消息（事件级 seq 定位 + 行/词高亮）；
- 中文分词走 `trigram`（插件在 patch 层覆盖核心配置），任意长度子串可召回；核心不支持该键时降级为 `unicode61` 的整词匹配。
